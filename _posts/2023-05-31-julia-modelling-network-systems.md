...
To build our networked system, the process is similar, except this time we need to use symbolic array variables. Note that `N` needs to have a well-defined value if I understand correctly. No symbolic values here.
...
- ModelingToolkit is pretty good, but doesn't seem to handle large matrices of parameters well, but maybe I'm using it wrong so it seems worth experimenting with. When debugging, `scalarize` is your friend.
